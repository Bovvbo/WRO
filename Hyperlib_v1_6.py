...
GKp = 38           # Proportional gain for heading correction during movement
GKd = 780           # Derivative gain for heading correction (currently disabled)
...
DistanceKp = 3    # Gain for distance control
...
DoubleTurnKp = 10  # Gain for two-motor turns
...
WheelD = 62.4        # Wheel diameter in mm
Axle = 200         # Axle length (distance between wheels) in mm
...
async def OdomUpdate():
    ...
    if abs(delta_theta_rad) > 1e-6:   # Use a small threshold for arc
        radius = delta_mm / delta_theta_rad
        chord = 2 * radius * sin(delta_theta_rad / 2)
        avg_theta = lastTheta + delta_theta / 2          # average heading during interval
        avg_theta_rad = radians(avg_theta)
        delta_x = chord * sin(avg_theta_rad)
        delta_y = chord * cos(avg_theta_rad)
    else:
        # Nearly straight: use current heading (or average heading for consistency)
...
async def TurnToPoint(TargetX, TargetY, MaxSpeed=1000, MinSpeed = 500, EarlyExit = 3, Forward = True, Direction = 0, Stopping = True):
    """
...
async def PurePursuit(MaxSpeed=900, MinSpeed=250, ArrivalThreshold=25, Lookahead=50,
                      Forward=True, DKp=DistanceKp, Stopping=True):
    global odomX, odomY, GKp, GKd, LoopTime, PArray, DistanceKp

    OneEighty = 0 if Forward else 180
    dSign = 1 if Forward else -1
    count = 0
    lastErr = 0  # Initialize derivative term
    LastX, LastY = PArray[-1]  # Final destination coordinates
    while count < len(PArray) - 1:
        
        # Get current segment waypoints
        x1, y1 = PArray[count]
        x2, y2 = PArray[count + 1]

        # Distance to next waypoint (for progression)
        distance_to_next = ((x2 - odomX) ** 2 + (y2 - odomY) ** 2) ** 0.5
        ...
        # Solve for intersection of lookahead circle with line segment
        b = 2 * (rx * dx + ry * dy)
        c = rx ** 2 + ry ** 2 - Lookahead ** 2
        disc = b ** 2 - 4 * a * c

        if disc < 0 or a == 0:  # No intersection or degenerate segment
            # Fallback: aim directly at the next waypoint
            finalX, finalY = x2, y2
        else:
            # Two possible intersections, take the one farther along the segment (larger t)
            sqrt_disc = disc ** 0.5
            t1 = (-b - sqrt_disc) / (2 * a)
            t2 = (-b + sqrt_disc) / (2 * a)
            t = max(t1, t2)
        ...
        # Vector from robot to the chosen pursuit point
        offsetX = finalX - odomX
        offsetY = finalY - odomY

        # Desired heading (using same convention as other movement functions)
        target_theta = degrees(atan2(offsetX, offsetY)) + OneEighty
        ...
        # Distance to final destination (for speed control)
        distanceToGo = ((LastX - odomX) ** 2 + (LastY - odomY) ** 2) ** 0.5

        # Speed always max until final point reached
        Speed = MaxSpeed
        # Apply movement
        await LeftControl(dSign * Speed, -correction)
        await RightControl(dSign * Speed, correction)
        ...
        if distance_to_next < ArrivalThreshold:
            count += 1  # Move to next segment

        # Allow time for motors to act and loop to run at consistent rate
        await wait(LoopTime)
    await MoveToPoint(PArray[-1][0], PArray[-1][1],
                  MaxSpeed=MaxSpeed, MinSpeed=MinSpeed,
                  ArrivalThreshold=ArrivalThreshold,
                  Forward=Forward, Stopping=Stopping)
